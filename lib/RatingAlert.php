...
require_once(_PS_MODULE_DIR_ . 'trustedshops/lib/TSCommon.php');
...
class RatingAlert
{
	...
	private static function _getAlertsInformations($iso)
	{
		$id_lang = Language::getIdByIso($iso);
		$cert = Configuration::get(TSCommon::PREFIX_TABLE . 'CERTIFICATE_' . strtoupper($iso));

		if ($cert != false) {
			$certificate = (array)Tools::jsonDecode(Tools::htmlentitiesDecodeUTF8($cert));

			if (trim($certificate['tsID']) != '') {
				$query = 'SELECT a.id_alert, a.`iso`, c.`email`, o.`id_order`, o.`id_lang` ' .
					'FROM `' . _DB_PREFIX_ . self::TABLE_NAME . '` a ' .
					'LEFT JOIN ' . _DB_PREFIX_ . 'orders o ON (a.id_order = o.id_order) ' .
					'LEFT JOIN ' . _DB_PREFIX_ . 'customer c ON (c.id_customer = o.id_customer) ' .
					'WHERE o.`id_lang`=' . (int)$id_lang . ' AND DATE_ADD(o.`date_add`, INTERVAL ' . (int)($certificate['send_seperate_mail_delay']) . ' DAY) <= NOW()';
	...
	public static function executeCronTask()
	{
		...
		if (is_array(TSCommon::$available_languages)) {
			...
			foreach (TSCommon::$available_languages as $iso => $iso_value) {

				$alerts_infos = RatingAlert::_getAlertsInformations($iso);

				///print_r($alerts_infos);

				if ($alerts_infos != false) {
					$common_count += sizeof($alerts_infos);

					foreach ($alerts_infos as $infos) {
						$cert = Configuration::get(TSCommon::PREFIX_TABLE . 'CERTIFICATE_' . strtoupper($infos['iso']));
						$certificate = (array)Tools::jsonDecode(Tools::htmlentitiesDecodeUTF8($cert));
		...
		return (sizeof($to_remove) == $common_count);
	}
